...
- 👍 up      → record only (mild positive signal)
- 💰 bought  → record only (strong positive signal; weighted more heavily)
- 👎 down    → record, 24h cooldown on the rule; 3 in 7d auto-disables the
                rule (enabled=0)

Positive feedback (👍/💰) does NOT change the rule's scan frequency. Instead
it raises the *push probability* of similar future tweets: the 30-day
aggregate is fed to ``decide_push_reason`` (signal_classifier), which lowers
the score threshold for rules the user has positively reinforced.

Per-post feedback rows live in ``sns_post_feedback`` so the time-series
can power both the 3-strike auto-disable and the push-probability boost.
Rule-level signals (cooldown, enabled) live on ``watch_rules`` and gate the
monitor loop.
"""
...
def record_sns_feedback(
    *,
    ...
    if kind in ("up", "bought"):
        # No rule-level mutation. Positive feedback does not change scan
        # frequency; the per-post row (recorded above) feeds the 30-day
        # aggregate that raises push probability for similar future tweets
        # via decide_push_reason (signal_classifier).
        logger.info(
            "SNS feedback %s rule_id=%s tweet_id=%s — recorded, no schedule change",
            kind, rule_id, tweet_id,
        )
        return result
